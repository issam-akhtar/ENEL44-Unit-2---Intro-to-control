...
\begin{figure}[htb]
    \begin{tikzpicture}
        ...
        \draw[->] (sum_n) -- node[above] {$y_m$} ++(1,0) -- ++(0,-2) -- (0,-2) -- (sum_r);
...
\begin{figure}[htb]
    \begin{tikzpicture}
        \node[circle,draw,minimum size=1cm] (sum_d) at (0,0) {};
        \node[rectangle,draw,minimum width=2cm,minimum height=1cm] (P) at (3,0) {$P$};
        \node[circle,draw,minimum size=1cm] (sum_n) at (6,0) {};
        \node (r) at (-2,0) {$r$};
        \node (d) at (0,2) {$d$};
        \node (n) at (6,2) {$n$};
